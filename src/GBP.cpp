...
Message calculate_factor_message(const Message &input_message, int target, const Message &factor_potential, int dim) {
    ...
    for (int i = 0; i < factor_potential.first.size(); i++) {
        if (factor_potential.first[i] == input_message.first) {
            index_variable = i;
        }
    ...
    lam.block(dim * index, dim * index, dim, dim) += input_message.second;

    if (index_tar != 0) {
        lam.block(0, 0, dim, dim).swap(lam.block(index_tar * dim, index_tar * dim, dim, dim));
        lam.block(0, 0, dim, lam.cols()).swap(lam.block(index_tar * dim, index_tar * dim, dim, lam.cols()));
    }
...
pair<vector<MatrixXd>, vector<MatrixXd>> Gaussian_Belief_Propagation(int dim, int num_states, const MatrixXd &Lambda) {
    vector<pair<VectorXd, MatrixXd>> factor(2 * num_states - 1);
    vector<pair<VectorXd, MatrixXd>> joint_factor(num_states - 1);
    ...
    for (int i = 0; i < 2 * num_states - 1; ++i) {
        ...
        if (i % 2 == 0) {
            ...
            joint_factor[var] = {variable, Lambda.block(dim * var, dim * (var + 2), 2 * dim, 2 * dim)};
            factor[i] = {variable, lam};
        }
    }
    ...
    for (int i = 0; i < num_states - 1; ++i) {
        auto variable_message = calculate_variable_message(factor_message[i], factor[2 * i]);
...
int main(){
    ...
    auto [sigma, sigma_joint] = Gaussian_Belief_Propagation(dim, num_states, lam);
