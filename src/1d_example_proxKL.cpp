...
#include "proxKL/ProxKLFactorizedSimpleGH.h"
#include "proxKL/ProxKL-GH.h"
...
int main(){
    ...
    std::vector<std::shared_ptr<ProxKLFactorizedSimpleGH>> vec_opt_fact;
    ...
    ProxKLGH<ProxKLFactorizedSimpleGH> opt{vec_opt_fact, dim_state, num_states, n_iters};
    opt.set_niter_low_temperature(n_iters);

    std::string source_root{XSTRING(SOURCE_ROOT)};
    std::string prefix{source_root+"/data/1d_proxKL/"};

    opt.update_file_names(prefix);
    std::string costmap_file{source_root+"/data/1d_proxKL/costmap.csv"};
