...
std::tuple<VectorXd, SpMat> NGDGH<FactorizedOptimizer, CudaClass>::compute_gradients(std::optional<double>step_size){
    Base::_Vdmu.setZero();
    Base::_Vddmu.setZero();
...
template <typename FactorizedOptimizer, typename CudaClass>
std::tuple<double, VectorXd, SpMat> NGDGH<FactorizedOptimizer, CudaClass>::onestep_linesearch(const double &step_size, 
                                                                        const VectorXd& dmu, 
...
template <typename FactorizedOptimizer, typename CudaClass>
double NGDGH<FactorizedOptimizer, CudaClass>::KL_Divergence(const VectorXd& mean_former, const VectorXd& mean_latter, const SpMat& precision_former, const SpMat& precision_latter)
{
    Timer timer;
...
template <typename FactorizedOptimizer, typename CudaClass>
inline void NGDGH<FactorizedOptimizer, CudaClass>::update_proposal(const VectorXd& new_mu, const SpMat& new_precision)
{
    Base::set_mu(new_mu);
 ...
 */
template <typename FactorizedOptimizer, typename CudaClass>
double NGDGH<FactorizedOptimizer, CudaClass>::cost_value_no_entropy()
{
