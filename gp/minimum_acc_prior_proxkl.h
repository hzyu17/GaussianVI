...
class MinimumAccGP_proxkl : public LinearFactor{
    public: 
        MinimumAccGP_proxkl(){};
        /**
         ...
         * @param delta_t 
         */
        MinimumAccGP_proxkl(const MatrixXd& Qc, double start_index, const double& delta_t, const VectorXd& mu_prior): 
        LinearFactor(),
        _dim{Qc.cols()},
        _dim_state{2*_dim},
        ...
        _Phi{_dim_state, _dim_state}{
            ...
            _target_mu = mu_prior.segment(start_index*_dim_state, 2 * _dim_state);
            ...
            // In the point robot case the condition doesn't match so we cannot make target_mu and Psi zero
            // Also the sign of Phi is wrong (But why the result was right in first iter)
        }
