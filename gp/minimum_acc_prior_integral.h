...
class MinimumAccGP_integral : public LinearFactor{
    public: 
        ...
        MinimumAccGP_integral(const MatrixXd& Qc, int start_index, const double& delta_t, const VectorXd& mu_0, int n_states, const std::vector<MatrixXd>& hA, const std::vector<MatrixXd>& hb): 
        LinearFactor(),
        _dim{Qc.cols()},
        _dim_state{2*_dim},
        _start_index{start_index},
        ...
        MatrixXd compute_Q (std::vector<MatrixXd> hb, int n_states){
            if (_start_index < n_states - 1)
                ...
                hb[_start_index + 1] * _Qc * hb[_start_index + 1].transpose())/2;
            else
                return _Phi * hb[_start_index] * _Qc * hb[_start_index].transpose() * _Phi.transpose();
        }
