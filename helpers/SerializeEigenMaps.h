...
namespace cereal {
    using Map = std::unordered_map<std::tuple<double, double>, std::tuple<Eigen::MatrixXd, Eigen::VectorXd>>;
    ...
    // template <class Archive>
    // void serialize(Archive& archive, Map& map) {
    //     size_t size = map.size(); // Get the size of the Map
    //     archive(size);           // Save the size information

    //     if (Archive::is_saving::value) {
    //         // Saving phase
    //         for (const auto& pair : map) {
    //             const auto& key = pair.first; // Key: std::tuple<double, double>
    //             const auto& value = pair.second; // Value: std::tuple<Eigen::MatrixXd, Eigen::VectorXd>
    //             archive(std::get<0>(key), std::get<1>(key)); // Serialize the two doubles in the key
    //             archive(std::get<0>(value), std::get<1>(value)); // Serialize the MatrixXd and VectorXd in the value
    //         }
    ...
    //             // Insert the deserialized key-value pair into the Map
    //             map.emplace(std::make_tuple(d1, d2), std::make_tuple(mat, vec));
    //         }
    //     }
    // }
